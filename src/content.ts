...
function mutationFunction() {
  ...
  var callback = function (mutationsList) {
    for (var index in mutationsList) {
      if (
        mutationsList[index].type == 'childList' &&
        mutationsList[index].target.nodeName.toLowerCase() != 'i'
      ) {
        nodeAdded();
      }
    }
...
function processJSON(data) {
  ...
  var wishAliasNames = data.wishlist
    ? loadAliases(data.wishlist, data.aliases, true)
    : [];
  ...
  chrome.storage.local.get('numgames', function (number) {
    ...
    if (numGames < data.response.game_count) {
      ...
      chrome.storage.local.set(
        {
          shortnames: ownedNames,
          numgames: data.response.game_count,
          aliases: newNames,
        },
        function () {
          ...
          if (wishedNames.length > 0 && wishAliasNames.length > 0)
            $.merge(wishedNames, wishAliasNames);
...
function addClassToElements(webName, webIndex, gameName, addedClass) {
  if (webName == gameName) {
    if (currentMode == HighlightMode.BUNDLE) {
      ...
      currentMode == HighlightMode.STORE ||
      currentMode == HighlightMode.SEARCH
    ) {
...
class SteamChecker {
  ...
  private currentMode: HighlightMode;
  ...
  constructor(mode: HighlightMode) {
    if (typeof chrome.storage.sync !== 'undefined') {
      this.storageType = chrome.storage.sync;
    } else {
      this.storageType = chrome.storage.local;
    }
  ...
  getUserData() {
    this.storageType.get(['steamid', 'id64'], (obj) => {
      if (obj.steamid) {
        var id64 = '';
        var wishlistTag = '';
        if (obj.id64) {
        ...
        xhrGames.open(
          'GET',
          'https://www.foxslash.com/apps/steamchecker/?steamid=' +
            obj.steamid +
            id64,
          true
        );
        xhrGames.onreadystatechange = function () {
          if (xhrGames.readyState == 4) {
            ...
            if (!data.message) {
              extConsoleOut(
                '(TIME) Steam Games: ' + (Date.now() - prevTime) + 'ms'
              );
        ...
        xhrWishlist.open(
          'GET',
          'https://www.foxslash.com/apps/steamchecker/wishlist.php?type=' +
            wishlistTag +
            '&id=' +
            obj.steamid,
          true
        ...
        xhrWishlist.onreadystatechange = function () {
          if (xhrWishlist.readyState == 4) {
            ...
            if (!data.message) {
              extConsoleOut(
                '(TIME) Steam Wishlist: ' + (Date.now() - prevTime) + 'ms'
              );
        ...
        xhrAliases.open(
          'POST',
          'https://www.foxslash.com/apps/steamchecker/aliases.php',
          true
        );
        xhrAliases.onreadystatechange = function () {
          if (xhrAliases.readyState == 4) {
            ...
            if (!data.message) {
              extConsoleOut(
                '(TIME) Steam Aliases: ' + (Date.now() - prevTime) + 'ms'
              );
        ...
        xhrAppList.open(
          'GET',
          'https://www.foxslash.com/apps/steamchecker/appList.json',
          true
        );
        xhrAppList.onreadystatechange = function () {
          if (xhrAppList.readyState == 4) {
            ...
            if (!data.message) {
              extConsoleOut(
                '(TIME) Steam App List: ' + (Date.now() - prevTime) + 'ms'
              );
        ...
        $.when(defGames, defAliases, defWishlist, defAppList).then(function () {
